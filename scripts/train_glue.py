...
cfgs = product(
    # skim_factor=[0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
...
for cfg in list(cfgs):
    folder = Path('/nvme2/yujiepan/workspace/moe-bert/moe-bert-learning/Transkimmer/model/glue', '0613-ft-start', f'factor{cfg.skim_factor}')
    task = Task(
        cmd=[f"""python -u run_glue_no_trainer.py \
            --model_type transkimer \
            --skim_coefficient {cfg.skim_factor} \
            --model_name_or_path yoshitomo-matsubara/bert-base-uncased-qnli \
...
Launcher(avail_cuda_list(3000)).run(tasks)
